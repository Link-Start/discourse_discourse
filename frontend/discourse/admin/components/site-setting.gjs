...
export default class SiteSettingComponent extends Component {
  ...
  syncFormValue = modifierFn((element, [wireValue]) => {
    if (this.formApi) {
  ...
  get settingFieldEntry() {
    return resolveSettingFieldType({
      type: this.setting.type,
      list_type: this.setting.list_type,
    });
  }
  ...
  get useFormKit() {
    return (
      this.trackChanges &&
      this.settingFieldEntry?.adminReady &&
      !this.settingEditButton &&
      typeof this.setting?.setting === "string"
    );
  }
        ...
        {{else}}
          {{#if this.useFormKit}}
            <Form
              ...
              {{this.syncFormValue this.buffered.value}}
